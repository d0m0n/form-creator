...
<div class="card max-w-[760px]">
    @if(isset($event))
    <form method="POST" action="{{ route('guest.event.update', $event) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf @method('PATCH')
    @else
    <form method="POST" action="{{ route('guest.event.store') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
    @endif
